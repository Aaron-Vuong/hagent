...
class Mapper:
    ...
    def pick(self, orig, norm, locs):
        kw = self.kw(orig)
        best = max(locs, key=lambda l: self.rank(l, kw))
    ...
    def kw(s):
        l = s.lower()
        out = set()
        if any(x in l for x in ('fpu', 'float', 'fp')):
            out.add('fp')
        if 'exu' in l:
            out.add('exu')
        if 'ld' in l or 'load' in l:
            out.add('ld')
        if 'st' in l or 'store' in l:
            out.add('st')
        return out
