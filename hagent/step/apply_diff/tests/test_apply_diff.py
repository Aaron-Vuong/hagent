...
"""
    ...
    monkeypatch.setattr(ChiselDiffApplier, 'apply_diff', lambda self, diff, original: original.replace('foo', 'baz'))

    step = Apply_diff()
    step.setup()
    data = {'chisel_original': orig, 'generated_diff': diff_text}
    out = step.run(data)
...
def test_apply_diff_fails_on_missing_removal(monkeypatch):
    ...
    monkeypatch.setattr(ChiselDiffApplier, 'apply_diff', lambda self, diff, original: original)
