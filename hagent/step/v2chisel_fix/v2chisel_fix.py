...
def diff_code(text1: str, text2: str) -> str:
    ...
    try:
        ...
        result = subprocess.run(
            ['diff', '-bBdNrw', '-U5', file1_name, file2_name], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
        )
...
class V2chisel_fix(Step):
    ...
    def run(self, data):
        ...
        if self.verilog_fixed_str.strip():
            ...
            diff_step.setup()
            tmp = diff_step.run({'verilog_original': self.verilog_original_str, 'verilog_fixed': self.verilog_fixed_str})
        ...
        for attempt in range(1, prompt3_iteration + 1):
            ...
            print(f'[V2ChiselFix] regenerated chisel_subset with metadata_context={self.meta}:\n{self.chisel_subset}')
    ...
    def _refine_diff_not_found(self, old_diff: str, applier_error: str) -> str:
        ...
        v2c_pass1.input_data = {'llm': self.template_config.template_dict.get('v2chisel_pass1', {}).get('llm', {})}
        v2c_pass1.setup()
        new_hints = v2c_pass1._extract_chisel_subset(self.chisel_original, self.verilog_diff_str, threshold_override=self.meta)
    ...
    def _generate_diff(self, old_code: str, new_code: str) -> str:
        ...
        diff_lines = difflib.unified_diff(
            old_lines, new_lines, fromfile='Original version', tofile='Modified version', lineterm=''
        )
        return '\n'.join(diff_lines)
